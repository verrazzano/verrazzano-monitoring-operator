...
func TestFilterDiffs01(t *testing.T) {
	input := `{
  ObjectMeta: {
   Name: "vmi-foo1-grafana",
   GenerateName: "",
   Namespace: "foo1",
-  SelfLink: "/apis/apps/v1/namespaces/foo1/deployments/vmi-foo1-grafana",
...
// Other examples of changes on the VMI spec
func TestCompare1(t *testing.T) {
	// Reducing the PVCs list of a live VMI - should get reported as a diff
	liveVMI := v1.VerrazzanoMonitoringInstance{}
	liveVMI.Spec.Elasticsearch.Storage.PvcNames = []string{"foo", "bar"}
	desiredVMI := v1.VerrazzanoMonitoringInstance{}
	desiredVMI.Spec.Elasticsearch.Storage.PvcNames = []string{"foo"}
	assert.NotEqual(t, "", CompareIgnoreTargetEmpties(liveVMI, desiredVMI), "Different sized PVC lists")
}
