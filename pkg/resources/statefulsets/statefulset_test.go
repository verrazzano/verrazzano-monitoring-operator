...
func verifyElasticSearch(t *testing.T, vmo *vmcontrollerv1.VerrazzanoMonitoringInstance,
	...
	assert.Equal("node.roles", env[1].Name, "Incorrect Env[1].Name")
	assert.Equal("master,data,ingest", env[1].Value, "Incorrect Env[1].Value")
	assert.Equal("cluster.name", env[2].Name, "Incorrect Env[2].Name")
	assert.Equal(vmo.Name, env[2].Value, "Incorrect Env[2].Value")
	assert.Equal("HTTP_ENABLE", env[3].Name, "Incorrect Env[3].Name")
	assert.Equal("true", env[3].Value, "Incorrect Env[3].Value")
	assert.Equal("logger.org.opensearch", env[4].Name, "Incorrect Env[4].Name")
	assert.Equal("info", env[4].Value, "Incorrect Env[4].Value")
	assert.Equal(constants.ObjectStoreAccessKeyVarName, env[5].Name, "Incorrect Env[5].Name")
	assert.Equal(constants.ObjectStoreAccessKey, env[5].ValueFrom.SecretKeyRef.Key, "Incorrect Env[5] Secret Key name")
	assert.Equal(constants.VerrazzanoBackupScrtName, env[5].ValueFrom.SecretKeyRef.Name, "Incorrect Env[5] Secret name")
	assert.Equal(constants.ObjectStoreCustomerKeyVarName, env[6].Name, "Incorrect Env[6].Name")
	assert.Equal(constants.ObjectStoreCustomerKey, env[6].ValueFrom.SecretKeyRef.Key, "Incorrect Env[6] Secret Key name")
	assert.Equal(constants.VerrazzanoBackupScrtName, env[6].ValueFrom.SecretKeyRef.Name, "Incorrect Env[6] Secret name")
