...
func TestVMIEmptyDeploymentSize(t *testing.T) {
	vmi := &vmcontrollerv1.VerrazzanoMonitoringInstance{}
	operatorConfig := &config.OperatorConfig{}
	deployments, err := New(vmi, operatorConfig, map[string]string{}, "vmi", "changeme")
	if err != nil {
		t.Error(err)
	}
...
func TestVMIFullDeploymentSize(t *testing.T) {
	vmi := &vmcontrollerv1.VerrazzanoMonitoringInstance{
	...
	assert.Equal(t, constants.VMIKind, deployments[0].ObjectMeta.OwnerReferences[0].Kind, "OwnerReferences is not set by default")
}
...
func TestVMIWithResourceConstraints(t *testing.T) {
	vmi := &vmcontrollerv1.VerrazzanoMonitoringInstance{
		Spec: vmcontrollerv1.VerrazzanoMonitoringInstanceSpec{
			Grafana: vmcontrollerv1.Grafana{
				Enabled: true,
	...
	for _, deployment := range deployments {
		...
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.Prometheus.Name) {
		...
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.AlertManager.Name) {
		...
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.Kibana.Name) {
		...
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.ElasticsearchIngest.Name) {
		...
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.ElasticsearchMaster.Name) {
			// No resources specified on this endpoint
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.ElasticsearchData.Name) {
			// No resources specified on this endpoint
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.Api.Name) {
			// No resources specified on API endpoint
		} else if deployment.Name == resources.GetMetaName(vmi.Name, config.PrometheusGW.Name) {
...
func TestVMIWithReplicas(t *testing.T) {
	...
	for _, deployment := range deployments {
		if deployment.Name == resources.GetMetaName(vmi.Name, config.Api.Name) {
...
func TestAPIWithNatGatewayIPs(t *testing.T) {
	...
	apiDeployment, err := getDeploymentByName(constants.VMIServiceNamePrefix+"my-vmi-api", deployments)
	if err != nil {
		t.Error(err)
	}
...
func TestWaitForElasticsearchTargetVersion(t *testing.T) {
	...
	kibana, _ := getDeploymentByName(resources.GetMetaName(vmi.Name, config.Kibana.Name), deployments)
