...
// StartHTTPServer runs an embedded HTTP server for any VMI handlers as a "resilient" goroutine meaning it runs in
...
func StartHTTPServer(controller *Controller) {
	...
	go wait.Until(func() {
		...
		if err != nil {
			glog.Errorf("Failed to start HTTP server for vmi: %s", err)
		}
