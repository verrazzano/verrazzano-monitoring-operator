...
func updateOpenSearchDashboardsDeployment(osd *appsv1.Deployment, controller *Controller, vmo *vmcontrollerv1.VerrazzanoMonitoringInstance) error {
	...
	if err != nil {
		metricsExporter.GetErrorMetrics(metricsExporter.Name_deploymentUpdateError).Inc()
...
func CreateDeployments(controller *Controller, vmo *vmcontrollerv1.VerrazzanoMonitoringInstance, pvcToAdMap map[string]string, existingCluster bool) (dirty bool, err error) {
	metricsExporter.GetFunctionMetrics(metricsExporter.Names_deployment).LogStart()
	defer metricsExporter.GetFunctionMetrics(metricsExporter.Names_deployment).LogEnd(false) //The error count is incremented by the function which calls createDeployment
...
func deleteDeployment(controller *Controller, vmo *vmcontrollerv1.VerrazzanoMonitoringInstance, deployment *appsv1.Deployment) error {
	metricsExporter.GetSimpleCounterMetrics(metricsExporter.Name_deploymentDeleteCounter).Inc()
	...
	if err != nil {
		metricsExporter.GetErrorMetrics(metricsExporter.Name_deploymentDeleteError).Inc()
...
func updateDeployment(controller *Controller, vmo *vmcontrollerv1.VerrazzanoMonitoringInstance, existingDeployment, curDeployment *appsv1.Deployment) error {
	metricsExporter.GetSimpleCounterMetrics(metricsExporter.Name_deploymentUpdateCounter).Inc()
	var err error
