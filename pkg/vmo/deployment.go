...
func CreateDeployments(controller *Controller, vmi *vmcontrollerv1.VerrazzanoMonitoringInstance, pvcToAdMap map[string]string, vmiUsername string, vmiPassword string) (dirty bool, err error) {
	...
	vmi.Spec.NatGatewayIPs = controller.operatorConfig.NatGatewayIPs

	deployList, err := deployments.New(vmi, controller.operatorConfig, pvcToAdMap, vmiUsername, vmiPassword)
	if err != nil {
		glog.Errorf("Failed to create Deployment specs for vmi: %s", err)
		return false, err
	}
	...
	glog.V(4).Infof("Creating/updating Deployments for vmi '%s' in namespace '%s'", vmi.Name, vmi.Namespace)
	for _, curDeployment := range deployList {
		...
		glog.V(6).Infof("Applying Deployment '%s' in namespace '%s' for vmi '%s'\n", deploymentName, vmi.Namespace, vmi.Name)
		existingDeployment, err := controller.deploymentLister.Deployments(vmi.Namespace).Get(deploymentName)
		if err != nil {
			if k8serrors.IsNotFound(err) {
				_, err = controller.kubeclientset.AppsV1().Deployments(vmi.Namespace).Create(context.TODO(), curDeployment, metav1.CreateOptions{})
			} else {
				return false, err
			}
		} else if existingDeployment != nil {
			if existingDeployment.Spec.Template.Labels[constants.ServiceAppLabel] == fmt.Sprintf("%s-%s", vmi.Name, config.Prometheus.Name) {
				prometheusDeployments = append(prometheusDeployments, curDeployment)
			} else if existingDeployment.Spec.Template.Labels[constants.ServiceAppLabel] == fmt.Sprintf("%s-%s", vmi.Name, config.ElasticsearchData.Name) {
				...
				if specDiffs != "" {
					...
					_, err = controller.kubeclientset.AppsV1().Deployments(vmi.Namespace).Update(context.TODO(), curDeployment, metav1.UpdateOptions{})
				}
			}
		}
		if err != nil {
	...
	prometheusDirty, err := updateNextDeployment(controller, vmi, prometheusDeployments)
	if err != nil {
		return false, err
	}
	elasticsearchDirty, err := updateNextDeployment(controller, vmi, elasticsearchDataDeployments)
	if err != nil {
		return false, err
	}
	...
	existingDeploymentsList, err := controller.deploymentLister.Deployments(vmi.Namespace).List(selector)
	if err != nil {
		return false, err
	}
	for _, deployment := range existingDeploymentsList {
		if !contains(deploymentNames, deployment.Name) {
			...
			err := controller.kubeclientset.AppsV1().Deployments(vmi.Namespace).Delete(context.TODO(), deployment.Name, metav1.DeleteOptions{})
			if err != nil {
...
func updateNextDeployment(controller *Controller, vmi *vmcontrollerv1.VerrazzanoMonitoringInstance, deployments []*appsv1.Deployment) (dirty bool, err error) {
	for index, curDeployment := range deployments {
		existingDeployment, err := controller.deploymentLister.Deployments(vmi.Namespace).Get(curDeployment.Name)
		if err != nil {
			return false, err
		}
