...
func CreatePersistentVolumeClaims(controller *Controller, sauron *vmcontrollerv1.VerrazzanoMonitoringInstance) (map[string]string, error) {
	...
	if err != nil {
		logger.Error().Msgf("Failed to create PVC specs for sauron: %s", err)
		return nil, err
	}
	deploymentToAdMap := map[string]string{}

	logger.Info().Msgf("Creating/updating PVCs for sauron '%s' in namespace '%s'", sauron.Name, sauron.Namespace)
	...
	for _, currPvc := range pvcList {
		...
		logger.Debug().Msgf("Applying PVC '%s' in namespace '%s' for sauron '%s'\n", pvcName, sauron.Namespace, sauron.Name)
